	...
	if (gl_HitKindEXT == 0) {
		...
		if (dotUp > 0) {
			...
			RAY_RECURSION_PUSH
				RAY_UNDERWATER_PUSH
					ray.color = vec4(0);
					traceRayEXT(tlas, 0, ~(RAYTRACE_MASK_HYDROSPHERE), 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, rayPosition, xenonRendererData.config.zNear, rayDirection, distanceToSurface, 0);
				RAY_UNDERWATER_POP
			RAY_RECURSION_POP
			...
			if (ray.hitDistance == -1) {
				...
				RAY_RECURSION_PUSH
					ray.color = vec4(0);
					traceRayEXT(tlas, 0, ~(RAYTRACE_MASK_HYDROSPHERE), 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, rayPosition, xenonRendererData.config.zNear, rayDirection, maxRayDistance, 0);
				RAY_RECURSION_POP
			...
			RAY_RECURSION_PUSH
				RAY_UNDERWATER_PUSH
					ray.color = vec4(0);
					traceRayEXT(tlas, 0, ~(RAYTRACE_MASK_HYDROSPHERE), 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, rayPosition, xenonRendererData.config.zNear, rayDirection, WATER_MAX_LIGHT_DEPTH, 0);
				RAY_UNDERWATER_POP
			RAY_RECURSION_POP
			if (ray.hitDistance == -1) {
				ray.hitDistance = maxLightDepth;
