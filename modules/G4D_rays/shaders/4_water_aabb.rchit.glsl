...
void main() {
	...
	// SetHitWater();
	// ray.color = vec4(vec3(0,0,1), 1);
	// return;
	
	
	const float waterWavesStrength = pow(0.5/*water.wavesStrength*/, 2) * smoothstep(500, 0, gl_HitTEXT);
	...
	if (gl_HitKindEXT == 0) {
		...
		if (waterWavesStrength > 0) {
			APPLY_NORMAL_BUMP_NOISE(WaterWaves, worldPosition, surfaceNormal, waterWavesStrength * 0.05 * smoothstep(1000, 100, gl_HitTEXT))
		}
		...
		RAY_RECURSION_PUSH
			ray.color = vec4(0);
			traceRayEXT(tlas, 0, ~(RAYTRACE_TYPE_WATER), 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, worldPosition, xenonRendererData.config.zNear, reflectDir, xenonRendererData.config.zFar, 0);
		RAY_RECURSION_POP
		...
		if (Refract(rayDirection, surfaceNormal, WATER_IOR)) {
			RAY_RECURSION_PUSH
				RAY_UNDERWATER_PUSH
					ray.color = vec4(0);
					traceRayEXT(tlas, 0, ~(RAYTRACE_TYPE_WATER), 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, worldPosition, xenonRendererData.config.zNear, rayDirection, WATER_MAX_LIGHT_DEPTH, 0);
				RAY_UNDERWATER_POP
			RAY_RECURSION_POP
		...
		if (dotUp > 0) {
			...
			RAY_RECURSION_PUSH
				RAY_UNDERWATER_PUSH
					ray.color = vec4(0);
					traceRayEXT(tlas, 0, ~(RAYTRACE_TYPE_WATER), 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, rayPosition, xenonRendererData.config.zNear, rayDirection, distanceToSurface, 0);
				RAY_UNDERWATER_POP
			RAY_RECURSION_POP
			...
			if (ray.hitDistance == -1) {
				...
				RAY_RECURSION_PUSH
					ray.color = vec4(0);
					traceRayEXT(tlas, 0, ~(RAYTRACE_TYPE_WATER), 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, rayPosition, xenonRendererData.config.zNear, rayDirection, maxRayDistance, 0);
				RAY_RECURSION_POP
			...
			RAY_RECURSION_PUSH
				RAY_UNDERWATER_PUSH
					ray.color = vec4(0);
					traceRayEXT(tlas, 0, ~(RAYTRACE_TYPE_WATER), 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, rayPosition, xenonRendererData.config.zNear, rayDirection, WATER_MAX_LIGHT_DEPTH, 0);
				RAY_UNDERWATER_POP
			RAY_RECURSION_POP
			if (ray.hitDistance == -1) {
				ray.hitDistance = maxLightDepth;
