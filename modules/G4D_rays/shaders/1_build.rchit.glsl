...
void main() {
	...
	if ((xenonRendererData.config.options & RENDER_OPTION_ACCUMULATE) != 0) {
		...
		if (recursions < RAY_MAX_RECURSION && RandomFloat(seed) < bounce_probabilities) {
			...
			RAY_RECURSION_PUSH
				traceRayEXT(tlas, 0, 0xff/*RENDERABLE_STANDARD*/, 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, rayOrigin, xenonRendererData.config.zNear, bounceDirection, xenonRendererData.config.zFar, 0);
			RAY_RECURSION_POP
		...
		RAY_RECURSION_PUSH
			RAY_GI_PUSH
				traceRayEXT(tlas, 0, 0xff/*RENDERABLE_STANDARD_EXCEPT_WATER*/, 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, facingWorldPosition, xenonRendererData.config.zNear, bounceDirection, 32, 0);
			RAY_GI_POP
		RAY_RECURSION_POP
		ray.color.rgb *= nDotL;
	...
	if (RAY_RECURSIONS < RAY_MAX_RECURSION) {
			...
			RAY_RECURSION_PUSH
				RAY_SHADOW_PUSH
					...
					for (int i = 0; i < MAX_SHADOW_TRANSPARENCY_RAYS; ++i) {
						traceRayEXT(tlas, 0, 0xff/*RENDERABLE_STANDARD_EXCEPT_WATER*/, 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, rayOrigin, shadowRayStart, sunDir, xenonRendererData.config.zFar, 0);
		...
		if (!rayIsGi) {
			...
			if (thisSurface.metallic + thisSurface.specular > 0) {
				...
				vec3 reflectDir = normalize(reflect(gl_WorldRayDirectionEXT, thisSurface.normal));
				RAY_RECURSION_PUSH
					traceRayEXT(tlas, 0, 0xff/*RENDERABLE_STANDARD*/, 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, worldPosition, xenonRendererData.config.zNear, reflectDir, xenonRendererData.config.zFar, 0);
				RAY_RECURSION_POP
			...
			if (thisSurface.color.a < 1) {
				...
				if (Refract(rayDirection, thisSurface.normal, thisSurface.ior)) {
					...
					if (Refract(rayDirection, thisSurface.normal, 1.0/thisSurface.ior)) {
						RAY_RECURSION_PUSH
							traceRayEXT(tlas, 0, 0xff/*RENDERABLE_STANDARD*/, 0/*rayType*/, 0/*nbRayTypes*/, 0/*missIndex*/, t2Position, 0, rayDirection, xenonRendererData.config.zFar, 0);
						RAY_RECURSION_POP
