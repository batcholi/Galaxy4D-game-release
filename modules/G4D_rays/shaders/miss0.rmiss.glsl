...
void main() {
	...
	// // Sunset
	// if (!rayIsGi)
	// 	ray.color.rgb += GetSunset() * pow(clamp(dot(gl_WorldRayDirectionEXT, renderer.sunDir), 0, 1), 2) * pow(1-abs(dot(gl_WorldRayDirectionEXT, vec3(0,1,0))), 4);
}
