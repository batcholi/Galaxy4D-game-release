...
STATIC_ASSERT_ALIGNED16_SIZE(RendererData, 3*64 + 8*8 + 4*16 + 8 + 8);
	...
	layout(buffer_reference, std430, buffer_reference_align = 4) buffer readonly IndexBuffer16 {uint16_t indices[];};
	...
	vec3 GetFlatNormalsFromVertices(uint instanceID, uint geometryID, uint triangleID) {
		GeometryData geometry = renderer.renderableInstances[instanceID].geometries[geometryID];
		uint index0 = triangleID * 3;
		uint index1 = triangleID * 3 + 1;
		uint index2 = triangleID * 3 + 2;
	...
	#ifdef SHADER_RCHIT
		vec3 GetFlatNormalsFromVertices() {
			return GetFlatNormalsFromVertices(gl_InstanceID, gl_GeometryIndexEXT, gl_PrimitiveID);
		}
	#endif
