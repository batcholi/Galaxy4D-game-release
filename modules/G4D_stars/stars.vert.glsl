...
void main() {
	...
	// Color
	vec3 color1 = pow(abs(RandomInUnitSphere(seed)) * vec3((1-abs(Simplex(pos/8e11))), abs(Simplex(pos/1e12)), abs(Simplex(pos/2e12))), vec3(0.333));
	...
	// Size
	gl_PointSize = clamp(RandomFloat(seed) * 5 + veryBrightStars + ultraBrightStars, 3, 7);
